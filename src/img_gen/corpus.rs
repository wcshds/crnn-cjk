...
pub fn get_random_chinese_text_with_font_list<'a, S1, S3>(
    ch_dict: &'a IndexMap<S1, Vec<AttrsOwned>>,
    weights: &WeightedAliasIndex<f64>,
    symbol: Option<&'a Vec<S3>>,
    range: RangeInclusive<u32>,
) -> Vec<(&'a str, Option<&'a Vec<AttrsOwned>>)>
where
    S1: AsRef<str>,
    S3: AsRef<str>,
{
...
pub fn wrap_text_with_font_list<'a, 'b, S1, S2>(
    text: &'a S1,
    ch_dict: &'b IndexMap<S2, Vec<AttrsOwned>>,
) -> Vec<(&'a str, Option<&'b Vec<AttrsOwned>>)>
where
    S1: AsRef<str> + ?Sized,
...
mod test {
    ...
    use crate::img_gen::{font_util::FontUtil, init::init_ch_dict_and_weight};
    ...
    fn test_wrap_text_with_font_list() {
        ...
        println!(
            "{:?}",
            wrap_text_with_font_list("這是一個測試", &ch_dict)[4]
        );
    }
}

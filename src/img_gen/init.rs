...
use crate::img_gen::font_util::FontUtil;
...
pub fn init_ch_dict_and_weight<'a, 'b>(
    font_util: &FontUtil,
    full_font_list: &'a Vec<AttrsOwned>,
    character_file_data: &'b str,
) -> (IndexMap<String, Vec<AttrsOwned>>, WeightedAliasIndex<f64>) {
    ...
    .unwrap();
    let ch_list: IndexMap<String, Vec<AttrsOwned>> = ch_list_and_weight
        .into_iter()
        .map(|(ch, _, font_list)| (ch.to_string(), font_list))
        .collect();
